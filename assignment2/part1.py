...
def plot_degree_distributions(G_actual, G_random):
    """
    Plot degree distributions of actual and random networks and save images
    Use non-interactive plotting, no pandas dependency needed
    """
    ...
    print(f"- Plotting complete, 3 visualization images saved, time elapsed: {time.time() - plot_start:.2f} seconds")
...
print("   - Actual network degree distribution shows heavy-tail properties, maximum degree significantly higher than random network")
print(
...
print("   - This indicates the existence of a few highly connected 'hub' nodes in the actual network, while in the random network, node degree distribution is more uniform")
...
if len(list(nx.connected_components(G_actual))) == 1:
    ...
    print(f"   - Largest connected component contains {G_actual_giant.number_of_nodes() / G_actual.number_of_nodes() * 100:.2f}% of the nodes")

print(f"   - Random network has {len(list(nx.connected_components(G_random)))} connected components")
print(f"   - Largest connected component contains {G_random_giant.number_of_nodes() / G_random.number_of_nodes() * 100:.2f}% of the nodes")